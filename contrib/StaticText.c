...
void alphaBlendTextSurfaceToFrame( SDL_Surface *textSurface, SDL_Texture *frameSurface, SDL_Rect *destRect );
...
//
  ...
  // 0xfede: FIX
//  uint8_t a_ = rendering == STRendering_Smooth ? 0XFF : 0X00;
//  uint8_t r,g,b,a;
//  SDL_GetRenderDrawColor(sdl2ctx.renderer, &r, &g, &b, &a);
//
//  SDL_SetRenderDrawColor(sdl2ctx.renderer, backColor->r, backColor->g, backColor->b, a_);
//
//  SDL_SetRenderTarget(sdl2ctx.renderer, frameP->frameSurfaceP);
//  SDL_RenderFillRect(sdl2ctx.renderer, NULL);
//
//  SDL_SetRenderDrawColor(sdl2ctx.renderer, r, g, b, a);
//  SDL_SetRenderTarget(sdl2ctx.renderer, NULL);
        ...
        err == kNoError )
  {
    ...
    if( err == kNoError )
    {
      ...
      uint32_t format;
      SDL_QueryTexture(frameP->frameSurfaceP, &format, NULL, NULL, NULL);
      
      convertedTextSurface = SDL_ConvertSurface(
                                                textSurface,
                                                SDL_AllocFormat(format),
                                                0 );
...
//
    ...
    if( err == kNoError )
    {
      if( rendering == STRendering_Blended || rendering == STRendering_Solid ) {
        destRect.h = frameP->frameRect.bottom;
        SDL_Texture* tx = SDL_CreateTextureFromSurface(sdl2ctx.renderer, textSurface);
        SDL_SetTextureBlendMode(tx, SDL_BLENDMODE_BLEND);
        
        SDL_SetRenderTarget(sdl2ctx.renderer, frameP->frameSurfaceP);
        SDL_RenderCopy(sdl2ctx.renderer, tx, NULL, &destRect);
        SDL_SetRenderTarget(sdl2ctx.renderer, NULL);
...
void alphaBlendTextSurfaceToFrame( SDL_Surface *textSurface, SDL_Texture *frameSurface, SDL_Rect *destRect )
{
  ...
  int h, w;
  uint32_t format;
  SDL_QueryTexture(frameSurface, &format, NULL, &w, &h);
  SDL_PixelFormat* txFormat = SDL_AllocFormat(format);
  
  void* txpixels;
  SDL_LockTexture(frameSurface, destRect, &txpixels, NULL);
  const char* str = SDL_GetError();
  
//  SDL_LockSurface( frameSurface );

  maxRow = h - destRect->y - 1;
  maxCol = w - destRect->x - 1;
  ...
  for( row=0; row<textSurface->h; row++ )
  {
    ...
    dst = &((Uint32*)txpixels)[ ( destRect->y + row ) * w + destRect->x ];

    for( col=0; col<textSurface->w; col++ )
    {
      if( col > maxCol ) break;

      if( (Uint32)(*src & textSurface->format->Amask) > (Uint32)(*dst & txFormat->Amask) )
        *dst = *src;
  ...
  SDL_UnlockTexture(frameSurface);
